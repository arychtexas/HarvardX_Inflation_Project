...
k <- tibble(Fed_Data)
colnames(All_Fed)<- c("Year", "Inflation_Rate_YoY", "Fed_Funds_Rate", 
                      "Business_Cycle", "GDP_Growth", 
                      "Events_Affecting_Inflation" )

p <-subset(All_Fed, select = -c(Inflation_Rate_YoY, Fed_Funds_Rate) )
d<- head(p,10)
d
...
ggplot(Final_CPI, aes(x = Annual_CPI_Average, y = Calendar_Year)) +
  ...
  annotate("text", x = 30.5, y = 2010, label = "LEGEND", size = 4, color = "grey")+
...
# With a r^2 of 22% this means that 78% of variance is explained by unknown factors. 
...
# With a r^2 of 0% this means that 100% of variance is explained by unknown factors. 
...
ggplot(All_CorrData, aes(x = Year, y = value, color = "red" ))+
  ...
  labs(
    title = " Pearson's (r) for Federal Funds Rate vs U.S. Inflation Rate YoY",
    subtitle = "HardvardX Capstone Project 2022",
    caption = "Portions of this data is from 1929-2017 and Reference Section.
    t = 4.9002, df = 87, p-value = 4.392e-06 alternative hypothesis: true correlation
    is not equal to 0. 95 percent confidence interval : 0.2843698 0.6138815 sample estimates: 
...
ggplot(FvCPIdata,aes(x, y)) +
  ...
  theme_classic() +
  labs(
    title = " Linear Regression: Fed Funds Rate vs Average U.S. CPI Annualized",
    subtitle = "HardvardX Capstone Project 2022",
    caption = "Portions of this data is from 1929-2017 and Reference Section.
...
# Our P-Value of .18 (18%) is well above our minimum of .05 (5%).  
#This means we have to alter our confidence interval  to 82.  
...
# Now that the data is smoothed and fits our current ARIMA model, 
# let us forecast what inflation would be in 10 years starting from 1993.  
...
#Verify our model by running check residual and update the conf. 
#This is a series of diagnostic tests that we will use to validate our model (Long, 2019).
...
# Let us compare the data and adjust the confidence interval to 87% based on the new P-Value of .13 (13%).
...
# Let's manually select the ARIMA.
...
#Let's update our confidence intervals to 92% based on our new p value.
...
#As we can see the majority of the rates are within our model. 
# In the Year 2000 was the Dot.com crash so that may explains why rates crashed downward.  
