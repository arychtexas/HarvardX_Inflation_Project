...
# Installation:
## Install Library(ies) if needed. Due to known loading issues, load the following package(s) dependencies, if needed
...
## Required data sets are embedded for download in Rmd.file, HTML, Github and PDF. The files will not be embedded in R Script! 

## If you are only using R script, please download the zip file from Inflation Case Rmd.file, HTML, PDF or GitHub (https://github.com/arychtexas/HarvardX_Inflation_Project):

Tools <- xfun::pkg_load2(c("htmltools", "mime")) 
Download_Link <- xfun::embed_files(c('Downloads/myrprojects/SP5001913.csv', 
                    'Downloads/myrprojects/US CPI.csv', 'Downloads/myrprojects/Inflation_Rate_Fed_Rate_1913_2017 - Sheet1.csv'))
Download_Link
if(interactive() ) htmltools::browsable(Download_Link)

# If you are having issues, after download,  you can use this code.
vroom('SP5001913.zip')

##############  FOR CHROMEBOOK USERS HAVING ISSUES ZIP BEGIN ################
# Note: For chromebook users, you may encounter an error. If so, please conduct the following:

#1. run this code 
Tools_1 <- xfun::pkg_load2(c("htmltools", "mime")) 

#2. Right click on the unzip file names in your directory. click get info. Copy and Paste the location inside of the parenthesis for each file ('enter file name and location'). Then run code.
Download_Link <- xfun::embed_files(c('enter file name and location', 
                                     'enter file name and location', 'enter file name and location'))

#3 run this code 
Download_Link
if(interactive() ) htmltools::browsable(Download_Link)
##############  FOR CHROMEBOOK USERS HAVING ISSUES ZIP END ##################

## Note: For reference, you can download the S&P 500 file from 
"https://datahub.io/core/s-and-p-500/r/data.csv"

## Load S&P 500 Dataset. 
# Note:  Insert your file path of where you saved the document 
SP500_Data <- read.csv(
  'Downloads/myrprojects/SP5001913.csv')

# Note: For reference,  Original US CPI file is located here 
"https://www.kaggle.com/datasets/varpit94/us-inflation-data-updated-till-may-2021?select=US+CPI.csv"

# Load US CPI Dataset. 
# Note: Insert your file path of where you saved the document 
CPI_Data <- read.csv(
  'Downloads/myrprojects/US CPI.csv')

# Note: For reference,  Original Fed Funds Rate data table is located here 
"https://www.thebalance.com/u-s-inflation-rate-history-by-year-and-forecast-3306093"

# Load US Fed Dataset. 
# Note:  Insert your file path of where you saved the document
Fed_Data <- read.csv(
  'Downloads/myrprojects/Inflation_Rate_Fed_Rate_1913_2017 - Sheet1.csv')

# Reference Section:

#1.Bond Yields and Interest Rates: 1900 to 2002. (2003). US CENSUS. Retrieved August 18, 2022, from https://www2.census.gov/library/publications/2004/compendia/statab/123ed/hist/hs-39.pdf
# The U.S. Census tracked The 3 Month Bond Yield from 1900 to 2002. The 3 Month Bond Yield is closely correlated with Federal Funds Rate. Use that data to fill in Federal Funds Rate.  

#2. Amadeo, K. (2022, July 27). US Inflation Rate by Year: 1929–2023. The Balance. Retrieved August 18, 2022, from [Inflation YoY] (Https://www.thebalance.com/u-s-inflation-rate-history-by-year-and-forecast-3306093)
...
#3. Irizarry, R. A. (2022, July 7). Introduction to Data Science. HARVARD Data Science. Retrieved August 8, 2022, from Https://rafalab.github.io/dsbook/
...
#4. Wheelock, D. C. (2021, September 13). Overview: The History of the Federal Reserve. Federal Reserve History. Retrieved August 8, 2022, from https://www.federalreservehistory.org/essays/federal-reserve-history
...
#5. Julian  G.F. (2022, May 10). U.S Inflation - Analysis in R. Kaggle. Retrieved August 8, 2022, from https://www.kaggle.com/code/fit4kz/u-s-inflation-analysis-in-r
...
#6.Standard and Poor’s (S&P) 500 Index Data including Dividend, Earnings and P/E Ratio. (n.d.). DataHub. Retrieved August 8, 2022, from https://datahub.io/core/s-and-p-500
...
#7. Bloomberg. (2022, August 15). Inside the Founding of the Federal Reserve [Video]. YouTube. https://www.youtube.com/watch?v=0hzdglWpxVM&t=314s
...
#8. U.S. Bureau of Labor Statistics. (2022). CPI Home : U.S. Bureau of Labor Statistics. Bls.gov. https://www.bls.gov/cpi/

#9 Standard and Poor’s 500 (S&P 500) - Explained. (n.d.). The Business Professor, LLC. Retrieved August 22, 2022, from https://thebusinessprofessor.com/en_US/investments-trading-financial-markets/standard-and-poors-500-sp-500-definition

#10 Introduction to ARIMA models. (2019). Duke.edu. https://people.duke.edu/~rnau/411arim.htm

#11 Long, J. (2019, September 26). 14 Time Series Analysis | R Cookbook, 2nd Edition. Retrieved September 5, 2022, from https://rc2e.com/timeseriesanalysis

#12 Srivastav, A. K. (2022, September 13). Pearson correlation coefficient. WallStreetMojo. Retrieved September 18, 2022, from https://www.wallstreetmojo.com/pearson-correlation-coefficient/ 

# Overview:
# Goal: 
# To examine if the United States' geopolitical, domestic, and economic events are correlated with Inflation Rate YoY.  I will also examine how the Federal Reserve Fund Rate affects the following: Monthly U.S. Consumer Price Index (CPI) average for all U.S. cities, Inflation Rate YoY, Geopolitical events, Economic Events, GDP Growth, and S&P 500 annualized prices utilizing the Pearson's Correlation Coefficient (r).  Create a forecasting machine learning model using back dated information to predict inflation and appropriate federal funds rate.
...
#Note:  The Standard & Poor's earliest origins can be linked to the stock market in 1923.  The Standard & Poor's index at the time contained 233 companies.  Today, it has 500 companies within its index.  It is widely tracked by economists, politicians, investors, and speculators.  It is often considered an early indicator of a possible economic expansion or slowdown. 
...
#Convert Annual Closing Price to dollars
...
wds <- head(WithDollarSign_SP500,20)
...
## Goal: 
#Create visualizations with individual data and combined data. Observe any inverse, none, or positive correlations.

#Chart of CPI Annualized from 1929 to 2017
...
ggplot(Final_CPI, aes(x = Annual_CPI_Average, y = Calendar_Year)) +
  ...
  coord_flip()+
  annotate("text", x = 19, y = 2015, label = "' --- Average CPI Annualized'",  size = 3, color = "blue")+
  annotate("text", x = 25.5, y = 2015, label = "LEGEND", size = 4, color = "grey")+
  annotate("text", x = 91 , y = 1945, label = " Average CPI Annualized ", size = 4, color = "blue")+
  annotate("text", x = 14, y = 2015, label = "' CPI '",  size = 3, color = "black")

#Chart of S&P 500 from 1929 to 2017 
...
#Chart to compare Annual CPI Average and S&P 500
...
Comp_Inflation_SP500 <- ggplot() +
  ...
  labs(
    title = " S&P 500 Closing Price Annualized vs CPI Accumulated Average Annualized",
    subtitle = "HardvardX Capstone Project 2022",
    caption = "Portions of this data is from the Reference Section. Chart scale is log10.",
    x = "S&P 500  Closing Price Annualized ($) x CPI Accumulated Average Annualized (%)",
    y = "Year (1929 - 2017)",
  )+
  scale_x_continuous(trans ="log10")+
  scale_y_continuous(trans = "log10")+
  ...
  theme(
    ...
    plot.subtitle = element_text(hjust = 0.5),
    plot.caption = element_text(face = "italic", hjust = 0.5)) +
  annotate("text", x = 10, y = 2014, label = "S&P 500", color = "red")+
  annotate("text", x = 12, y = 2014, label = "CPI", color = "blue")+
  annotate("text", x = 15, y = 2014, label = "LEGEND", size = 4, color = "black")+
  annotate("text", x = 500, y = 1982, label = "81' Highest Fed Funds Rate to battle inflation", size = 3, color = "black")+
  annotate("text", x = 30, y = 1935, label = "29-33'Great Depresion & Deflation", size = 3, color = "black")+
  annotate("text", x = 2200, y = 2007, label = "07-09' The Great Recession ", size = 3, color = "black")+
  annotate("text", x = 150, y = 1970, label = "69-79' Lost Decade(Runaway Inflation)", size = 3, color = "black")+
  annotate("text", x = 70, y = 1953, label = "51' Federal Reserve Accord", size = 3, color = "black")+
  coord_flip()
...
#Chart of Federal Funds Rate from 1929 - 2017

# The Federal Funds Rate is a tool that is utilize by the federal reserve to help spur economic growth or slowdown. 
...
#Chart of Federal Reserve's Fed Funds Rate and Inflation Rate YoY 

# Federal Reserve's Fed Funds Rate is a tool utilized by the Federal Reserve to tackle inflation, economic slowdowns or promote growth in the economy.  Inflation at high levels is one of the most significant issues that can cause an economic slowdown.  
...
ggplot(data = All_Fed, aes( x = `Year`))+
  ...
  theme(
    ...
    plot.subtitle = element_text(hjust = 0.5),
    plot.caption = element_text(face = "italic", hjust = 0.5))+
  annotate("text", x = 2015, y = -8, label = "'Inflation Rate YoY'", size = 3, color = "red")+
  annotate("text", x = 2015, y = -9, label = "'Fed Funds Rate'",  size = 3, color = "lightblue4")+
  annotate("text", x = 2015, y = -7, label = "LEGEND", size = 4, color = "black")
...
# Chart of GDP Growth with the Average GDP

# Negative GDP signals economic slowdown, a neutral rate indicates economic stagnation, and a positive rising GDP rate signals economic expansion.  Let's take a look at GDP.
...
ggplot(data = All_Fed, aes( x = `Year`))+
  ...
  annotate("text", x = 2015, y = -10, label = "' --- Average GDP Growth'",  size = 3, color = "gold4")+
  annotate("text", x = 2015, y = -9, label = "LEGEND", size = 4, color = "black")+
  annotate("text", x = 2011 , y = 4.3, label = " Average GDP Growth ", size = 3, color = "gold4")


#Chart of GDP Growth and Inflation Rate YoY  
ggplot(data = All_Fed, aes( x = `Year`))+
  ...
  theme(
        ...
        plot.subtitle = element_text(hjust = 0.5),
        plot.caption = element_text(face = "italic", hjust = 0.5))+
  annotate("text", x = 2015, y = -10, label = "'Inflation Rate YoY'", size = 3.5, color = "red")+
  annotate("text", x = 2015, y = -12, label = "'GDP Growth'",  size = 3.5, color = "green")+
  annotate("text", x = 2015, y = -8, label = "LEGEND", size = 4, color = "black")+
  annotate("text", x = 2015, y = -14.2, label = "'Data Overlap'",  size = 3.5, color = "dark green")
...
## Goal: 
## To observe if the Fed Funds Rate has a positive, negative or no correlation with Inflation YoY, GDP Growth and CPI Average Annualized from 1929 - 2017. 
...
All_CorrData %>%
  gt() %>%
  tab_header(
    title = md("**Variables for Pearson's Correlation Data Examination**"),
    subtitle = md("HardvardX Capstone Project 2022")
  ) %>%
  cols_width(
    everything() ~ px(160)) %>%
...
Initial_Pearson_Results %>%
  gt() %>%
  tab_header(
    title = md("** Pearson's Correlation Data Variable Variation (%)**"),
    subtitle = md("HardvardX Capstone Project 2022")
  ) %>%
  cols_width(
    everything() ~ px(160)) %>%
...
ggplot(data,aes(x, y)) +
  ...
  theme_classic() +
  labs(
    title = " Linear Regression: Federal Funds Rate vs U.S. Inflation Rate YoY",
    subtitle = "HardvardX Capstone Project 2022",
...
# Linear Regression Chart of the Pearson's (r) for Fed Funds Rate and Inflation Rate YoY from 1929-2017
...
ggplot(All_CorrData, aes(x = Year, y = value, color = "red" ))+
  ...
  labs(
    title = " Pearson's Correlation Coefficient (r) Formula for Federal Funds Rate vs U.S. Inflation Rate YoY",
    subtitle = "HardvardX Capstone Project 2022",
    caption = "Portions of this data is from 1929-2017 and Reference Section.
    t = 4.9002, df = 87, p-value = 4.392e-06 alternative hypothesis: true correlation
    is not equal to 0. 95 percent confidence interval: 0.2843698 0.6138815 sample estimates: 
...
ggplot(FvGDPdata,aes(x, y)) +
  ...
  theme_classic() +
  labs(
    title = " Linear Regression: Federal Funds Rate vs U.S. GDP Growth",
    subtitle = "HardvardX Capstone Project 2022",
    caption = "Portions of this data is from 1929-2017 and Reference Section.
...
# Linear Regression Chart of the Pearson's (r) for Fed Funds Rate and GDP Growth from 1929-2017
...
ggplot(All_CorrData, aes(x = Year, y = value, color = "red" ))+
  ...
  labs(
    title = " Pearson's Correlation Coefficient (r) Formula for Federal Funds Rate vs U.S. GDP Growth from 1929-2017",
    subtitle = "HardvardX Capstone Project 2022",
    caption = "Portions of this data is from 1929-2017 and Reference Section. 
    ...
    sample estimates: cor  -0.0311815",
    x = "Year (1929-2017)",
    y = "Fed Funds Rate (%) x GDP Growth Correlation (%)",
  )+
  theme_classic()+
  theme(legend.title=element_blank(),
        legend.position = 'bottom',
...
ggplot(FvCPIdata,aes(x, y)) +
  ...
  theme_classic() +
  labs(
    title = " Linear Regression: Federal Funds Rate vs Average U.S. CPI Accumulated Annualized",
    subtitle = "HardvardX Capstone Project 2022",
    caption = "Portions of this data is from 1929-2017 and Reference Section.
...
# Linear Regression Chart of the Pearson's (r) for Fed Funds Rate and Annual CPI Average from 1929-2017

ggplot(All_CorrData, aes(x = Year, y = value))+
  geom_point(aes(y = `Fed Funds Rate`, col = 'Fed Funds Rate'))+
  geom_point(aes(y = `Annual CPI Average`, col = "Annual CPI Average"))+
  labs(
    title = " Pearson's Correlation Coefficient (r) Formula for Federal Funds Rate vs Average U.S. CPI Accumulated Annualized' from 1929-2017",
    subtitle = "HardvardX Capstone Project 2022",
    ...
    x = "Year (1929-2017)",
    y = "Fed Funds Rate (%) x Average CPI Accumulated Annualized (%)",
  )+
  theme_classic()+
  theme(legend.title=element_blank(),
        legend.position = 'bottom',
...
ggplot(data = Conclusion_All_Fed, aes(x = The_Year))+
  ...
  labs(
    ...
    x = "Year (1929 - 2017)",
    y = "Federal Funds Rate(%) x Average GDP* x Inflation Rate YoY(%)"
  )+
  ...
  theme(
    ...
    plot.caption = element_text(face = "italic", hjust = 0.5))+
  annotate("text", x = 2015, y = -8, label = "'Inflation Rate YoY'", size = 3, color = "red")+
  annotate("text", x = 2015, y = -9.5, label = "'Fed Funds Rate'",  size = 3, color = "lightblue4")+
  annotate("text", x = 2015, y = -6, label = "LEGEND", size = 4, color = "black")+
  annotate("text", x = 1980, y = 18.5, label = "81' Highest Fed Funds Rate to battle inflation", size = 3, color = "black")+
  annotate("text", x = 1953, y = 18, label = "46' Budget cuts post WWII", size = 3, color = "black")+
  annotate("text", x = 1934, y = 17.9, label = "39-45' WWII(highest GDP)", size = 3, color = "black")+
  annotate("text", x = 1932, y = -11.5, label = "29-33'Great Depresion & Deflation", size = 3, color = "black")+
  annotate("text", x = 2008, y = 6, label = "07-09' The Great Recession ", size = 3, color = "black")+
  annotate("text", x = 1970, y = 13.5, label = "69-79' Lost Decade(Runaway Inflation)", size = 3, color = "black")+
  annotate("text", x = 2015, y = -10.9, label = "'GDP >=3.38'",  size = 3, color = "dark green")+
...
ggplot() +
  ...
  theme(
    ...
    plot.caption = element_text(face = "italic", hjust = 0.5)) +
  coord_flip()+
  annotate("text", x = -12, y = 2018, label = "Fed Funds Rate", color = "red")+
  annotate("text", x = -10, y = 2018, label = "Inflation Rate YoY", color = "blue")+
  annotate("text", x = -9, y = 2018, label = "LEGEND", size = 4, color = "black")+
...
#Lets see if the correlation changes if I remove 1929-1950 data.
...
ggplot(NEW_1951_data,aes(x, y)) +
  ...
  theme_classic() +
  labs(
    title = " Linear Regression: Federal Funds Rate vs U.S. Inflation Rate YoY (1951-2017)",
    subtitle = "HardvardX Capstone Project 2022",
...
ggplot(a1951_Fed, aes(x = Year, y = value, color = "red" ))+
  ...
  geom_smooth(aes(y = `Inflation Rate YoY`, color = 'Inflation Rate YoY' ), se = FALSE) +
  labs(
    title = " Pearson's Correlation Coefficient (r) Formula for Federal Funds Rate vs U.S. Inflation Rate YoY (1951-2017)",
    subtitle = "HardvardX Capstone Project 2022",
    caption = "Portions of this data is from 1951-2017 and the Reference Section. 
    t = 9.7623, df = 65, p-value = 2.299e-14 alternative hypothesis: true correlation
    is not equal to 0. 95 percent confidence interval: 0.6515102 0.8532300 sample estimates: 
...
Final_Pearson_Results %>%
  gt() %>%
  tab_header(
    title = md("Final Pearson's Correlation Variable Variation Data (%)"),
    subtitle = md("HardvardX Capstone Project 2022")
  ) %>%
  cols_width(
    everything() ~ px(160)) %>%
...
#Data Analysis: Creating a Machine Learning Inflation and Federal Reserve Fund Rate Forecasting Model. 
...
#Now that we have the data properly formatted, we have to verify if the data is stationary.  Per the Duke research team ("Introduction to ARIMA models," 2019), "A stationary series has no trend, its variations around its mean have a constant amplitude, and it wiggles consistently, i.e., its short-term random time patterns always look the same in a statistical sense.  With that being established, the Armia model requires stationary data to predict future values from older data properly."  In this case, we are using data from 1929-2017.  We will predict ten years from 2018-2028 and backtest the data from 1993-2003. 

# First, let us backtest the data to find a suitable model to predict future values.  We will do a ten year model from 1993-2003.
...
# ACF shows us similarities over time using lagged data in a time series.  The autocorrelations within the blue upper and lower limits are considered significant.  The insignificant autocorrelations exceed the blue upper and lower limits.  In this ACF data plot, more data units pass the blue upper line, indicating that the data is not as stationary.  Lag is a specific period of time; we will reference the lag with a number, i.e., lag 1.
...
autoplot(Backtest_Inflation_Model_Forecast, ts.colour = 'coral', predict.colour = 'green4', predict.linetype = 'dashed', conf.int = TRUE, conf.int.linetype = "dashed", conf.int.fill = "azure")+
...
#Let's examine how many years are within 1000 basis points (1 point) of the original Fed Data object Inflation_Model.
...
autoplot(Backtest_Inflation_Model_Forecast, ts.colour = 'coral', predict.colour = 'green4', predict.linetype = 'dashed', conf.int = TRUE, conf.int.linetype = "dashed", conf.int.fill = "azure")+
  ...
  annotate("text", x = 1970, y = -13, label = "Model YoY Rate", color = "green4")+
...
# Great improvement.  Per (Long, 2019), When verifying our check residuals graphs, we should look for the following: 1.  Do the standardized residuals show volatility clusters?  If so, readjust the model.  2.  Does the autocorrelation function (ACF) show significant autocorrelation between the residuals?  If so, readjust the model. 3.  Do the residuals look bell-shaped?  If so, this suggests they are reasonably symmetrical.  4.  Are the p-value in the Ljung–Box test extensive?  If so, this indicates the residuals are patternless—meaning the model has extracted all the information, and the only noise is left behind. 
...
# - Note: row 3 (1995 comparison) is actually False. 
autoplot(Backtest_Inflation_Model_Forecast_1, ts.colour = 'coral', predict.colour = 'green4', predict.linetype = 'dashed', conf.int = TRUE, conf.int.linetype = "dashed", conf.int.fill = "azure")+
  ...
  annotate("text", x = 1970, y = -12.5, label = "Model YoY Rate", color = "green4")+
...
autoplot(Inflation_Model_Forecast_Update, ts.colour = 'coral', predict.colour = 'green4', predict.linetype = 'dashed', conf.int = TRUE, conf.int.linetype = "dashed", conf.int.fill = "azure")+
...
#Chart the Model
autoplot(Backtest_Fed_Model_Forecast, ts.colour = 'darkgoldenrod', predict.colour = 'green4', predict.linetype = 'dashed', conf.int = TRUE, conf.int.linetype = "dashed", conf.int.fill = "azure")+
...
#This backtest model is only computing the value 3 as a prediction. We want the prediction within 1.01 percentage points or 1000 basis points of the original Fed Model object Fed_Fund_Rate.  After viewing the results from check residuals, we noticed that the data didn't check all the boxes referenced above and P-Value is .71 (71%) which gives us a confidence level of 29%.  Let's manually select the ARIMA.
...
#Chart the Data
autoplot(Backtest_Fed_Model_Forecast_Update, ts.colour = 'darkgoldenrod', predict.colour = 'green4', predict.linetype = 'dashed', conf.int = TRUE, conf.int.linetype = "dashed", conf.int.fill = "azure")+
...
#Verify model accuracy (1000 basis points of the original Fed Model object Fed_Fund_Rate). 
...
# Any improvements?
GoodFed<- Compare_Fed$Fed_Diff<=1.01
length(GoodFed[GoodFed== TRUE])
length(GoodFed[GoodFed== FALSE])

Compare_Fed$new_col <- GoodFed
colnames(Compare_Fed)<- c("Year", "Original_Fed_Rate", "Model_Fed_Fund_Rate", "Fed_Diff", "Less_Than_1001_Basis_Points")
...
Compare_Fed %>%
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = Less_Than_1001_Basis_Points,
      rows = (1:8)
    )
  ) %>%
  tab_style(
...
autoplot(Backtest_Fed_Model_Forecast_Update, ts.colour = 'darkgoldenrod', predict.colour = 'green4', predict.linetype = 'dashed', conf.int = TRUE, conf.int.linetype = "dashed", conf.int.fill = "azure")+
  ...
  theme_classic() +
  labs(
    title = " Backtest Model Federal Funds Rate vs Actual Federal Funds Rate (1993-2003)",
    subtitle = "HardvardX Capstone Project 2022",
    caption = "Portions of this data is from 1929-2017 and the Reference Section.",
    x = "Years",
  ...
  annotate("text", x = 1970, y = -3.5, label = "Model Fed Rate", color = "green4")+
  annotate("text", x = 1970, y = -2.5, label = "LEGEND", size = 4, color = "black")

#As we can see the majority of the rates are within our model. In the Year 2000 was the Dot.com crash so that may explains why rates crashed downward.  
...
#Chart Data
autoplot(Fed_Model_Forecast, ts.colour = 'darkgoldenrod', predict.colour = 'green4', predict.linetype = 'dashed', conf.int = TRUE, conf.int.linetype = "dashed", conf.int.fill = "azure")+
...
#* Backtest Code: Inflation_Model_1993_2003 = arima(fixed = NULL, Inflation_Model_1993_Time, order = c(6,2,1), transform.pars=TRUE)
#Backtest_Inflation_Model_Forecast_1 = forecast(Inflation_Model_1993_2003, level = c(87), h = 11)
#* Forecast Code: True_Inflation_Model = arima(fixed = NULL, Inflation_Model_Time, order = c(6,2,1), transform.pars=TRUE)
#Inflation_Model_Forecast_Update = forecast(True_Inflation_Model, level = c(90), h = 11)
...
#* Backtest Code: Fed_Model_1993_2003 = arima(fixed = NULL, Fed_Model_1993_Time, order = c(5,1,6),  transform.pars=TRUE)
#Backtest_Fed_Model_Forecast_Update = forecast(Fed_Model_1993_2003, level = c(92), h = 11)
#* Forecast Code: Fed_Model_True = arima(fixed = NULL, transform.pars=TRUE, Fed_Model_Time, order = c(5,1,6))
#Fed_Model_Forecast = forecast(Fed_Model_True, level = c(95), h = 11)
###############################################################################
## Conclusion 

# Utilizing Pearson's "r" correlation tools can show how different variables may have zero, inverse or positive correlation.  In this case, we know that economic and geopolitical factors can drive outliers affecting Pearson's "r" calculations by skewing the calculated results. 
